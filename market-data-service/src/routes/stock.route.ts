...
router.post("/", requireAuth, verifyAdmin, asyncHandler(stockController.createStock)); //Admin only
//Record new price for stock with given symbol [Admin only]
router.post("/:symbol/price", requireAuth, verifyAdmin, asyncHandler(stockController.recordStockPrice)); //Admin only
...
router.get("/:symbol/price", requireAuth, asyncHandler(stockController.getLatestPrice));
//Get price history for stock with given symbol, with optional query params for date range (start_date, end_date)
router.get("/:symbol/history", requireAuth, asyncHandler(stockController.getPriceHistory));

//Update stock details by id [Admin only] - can update symbol, company_name, exchange
router.put("/:id", requireAuth, verifyAdmin, asyncHandler(stockController.updateStock)); //Admin only
//Delete stock by id [Admin only]
router.delete("/:id", requireAuth, verifyAdmin, asyncHandler(stockController.deleteStock)); //Admin only
